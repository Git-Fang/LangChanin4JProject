...
1. **创建应用程序面板**：
   ...
   - 配置查询语句，如：
     ```
     sum(http_server_requests_seconds_count{job="rag-translation"}) by (uri)
     ```
   - 配置图表类型、标题等
   - 点击 "Apply"
...
2. **创建系统资源面板**：
   ...
   - 配置查询语句，如：
     ```
     process_cpu_usage{job="rag-translation"}
     ```
   - 配置图表类型、标题等
   - 点击 "Apply"
