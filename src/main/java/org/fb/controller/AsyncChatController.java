...
public class AsyncChatController implements EnvironmentAware {
    ...
    /**
     * 从 ChatRequestMessage 中提取 modelId
     */
    private String extractModelId(ChatRequestMessage request) {
        if (request != null && request.getMetadata() != null) {
            return request.getMetadata().getModelId();
        }
        return null;
    }
    
    /**
     * 处理同步请求（从 async 端点调用，不带 modelId）
     */
    private void processSynchronously(ChatRequestMessage request) {
        processSynchronously(request, null);
    }
    
    /**
     * 处理同步请求（支持 modelId）
     * @param request 请求消息
     * @param explicitModelId 显式指定的模型ID（优先级高于 request 中的）
     */
    private void processSynchronously(ChatRequestMessage request, String explicitModelId) {
        // 优先使用显式指定的 modelId，否则从请求中提取
        String modelId = explicitModelId != null ? explicitModelId : extractModelId(request);
        
        // 记录模型切换日志
        if (modelId != null && !modelId.isEmpty()) {
            log.info("【模型切换】使用模型ID: {} 进行流式聊天", modelId);
        }
        
        if (streamingDispatchService != null) {
            log.info("[Standalone模式] 使用流式分发服务(意图识别+业务分发), requestId: {}, modelId: {}", request.getRequestId(), modelId);
            ...
            try {
                ...
                Flux<String> flux = streamingDispatchService.chat(request.getMemoryId(), request.getMessage(), modelId);
...
flux.publishOn(Schedulers.boundedElastic())
    ...
    @Operation(summary = "HTTP流式聊天", description = "通过POST发送消息，SSE流式返回结果，支持动态模型切换")
    public Map<String, Object> startHttpStreamChat(@RequestBody ChatForm chatForm) {
        ...
        log.info("==================== HTTP Stream 请求开始 ====================");
        log.info("【HTTP Stream】memoryId: {}, 模型ID: {}, 用户消息: {}", 
                memoryId, 
                modelId != null ? modelId : "(默认)",
                userMessage != null && userMessage.length() > 50 ? userMessage.substring(0, 50) + "..." : userMessage);
        
        if (extractedTexts != null && !extractedTexts.isEmpty()) {
            log.info("【HTTP Stream】附带文件提取内容数量: {}", extractedTexts.size());
        }

        // 记录模型切换日志
        if (modelId != null && !modelId.isEmpty()) {
            log.info("【模型切换】HTTP Stream 使用模型: {}", modelId);
        }
        ...
        try {
            // 创建请求消息，传入 modelId
            ChatRequestMessage request = ChatRequestMessage.create(memoryId, fullMessage, modelId);
            String requestJson = objectMapper.writeValueAsString(request);
            ...
            log.info("【HTTP Stream】请求数据已保存, requestId: {}, Redis保存: {}", requestId, redisSaved);
            ...
            // 启动同步处理（传递 modelId）
            if (isStandalone()) {
                log.info("[Standalone模式] HTTP Stream 同步处理请求, modelId: {}", modelId);
                processSynchronously(request, modelId);
            }
        } catch (Exception e) {
            log.error("保存请求数据失败, requestId: {}, modelId: {}", requestId, modelId, e);
            // 异常时也使用内存缓存作为降级方案
            try {
                ChatRequestMessage request = ChatRequestMessage.create(memoryId, fullMessage, modelId);
                memoryCache.put(requestId, request);
        ...
        try {
            ...
            if (requestJson == null) {
                ...
                if (request != null) {
                    ...
                    tempModelId = request.getMetadata() != null ? request.getMetadata().getModelId() : null;
                } else {
                    // 内存缓存也没有时，使用降级处理
            ...
            final String modelId = tempModelId;

            // 记录模型切换日志
            if (modelId != null && !modelId.isEmpty()) {
                log.info("【模型切换】SSE流使用模型: {}", modelId);
            }

            if (streamingDispatchService != null) {
                log.info("使用流式分发服务(意图识别+业务分发)处理HTTP请求, requestId: {}, modelId: {}", requestId, modelId);

                // 安全设置Redis值
                safeSetRedisValue(STREAM_CACHE_PREFIX + requestId, "", RESULT_TTL);

                Flux<String> flux = streamingDispatchService.chat(memoryId, message, modelId);
